...
func TestSnap_snapPolygon(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			tmIDs:  []tms20.TMID{14},
			config: processing.Config{KeepPointsAndLines: true},
			polygon: geom.Polygon{{
		...
		},
		{
			...
			tms:     newSimpleTileMatrixSet(0, 1),
			config:  processing.Config{IgnoreOutsideGrid: true},
			tmIDs:   []tms20.TMID{0},
			polygon: geom.Polygon{{{0.1, 0.1}, {0.2, 0.1}, {0.2, -0.1}}},
			want:    map[tms20.TMID][]geom.Geometry{}, // empty, ignored
		},
		{
			...
			tms:    newSimpleTileMatrixSet(2, 64),
			config: processing.Config{ReverseWindingOrder: false},
			tmIDs:  []tms20.TMID{1},
			polygon: geom.Polygon{
		...
		},
		{
			...
			tms:    newSimpleTileMatrixSet(2, 64),
			config: processing.Config{ReverseWindingOrder: true},
			tmIDs:  []tms20.TMID{1},
			polygon: geom.Polygon{
				{{4.0, 124.0}, {4.0, 4.0}, {60.0, 4.0}, {60.0, 124.0}},                   // ccw
				{{12.0, 116.0}, {52.0, 116.0}, {52.0, 76.0}, {28.0, 76.0}, {12.0, 76.0}}, // cw
		...
		t.Run(tt.name, func(t *testing.T) {
			if tt.wantPanic {
				require.Panics(t, func() {
					...
					SnapGeometry(ix, tt.polygon, tt.tmIDs, tt.config)
				})
				return
			}
			...
			got := SnapGeometry(ix, tt.polygon, tt.tmIDs, tt.config)
